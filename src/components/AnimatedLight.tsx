...
const AnimatedLight = ({ id, onFocus, name, brightness, color, isGroup, isOn, focused }: LightProps): JSX.Element => {
    const navigate = useNavigate();
    // const [stateOn, setStateOn] = React.useState(isOn);
    const brightnessHeight = (switchSize * brightness) * 0.01;
    const displayBrightness = isOn ? `${brightness}% Animated Brightness` : 'Turned off';
...
const FocusableItem = withFocusable()(AnimatedLight);
