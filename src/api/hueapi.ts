...
export const getGroups = async (): Promise<Array<Room> | null> => {
  ...
  if (base) {
    ...
    if (groups && groups[0] && groups[0].error) {
      console.error(
        ` >>>>>>>>>>>>>>>>>>>>>>>>>>>>> no groups found. Error: ${JSON.stringify(
          groups,
          null,
          "    "
        )} `
...
export const getLights = async (): Promise<Array<Light> | null> => {
  ...
  if (base) {
    ...
    if (lights && lights[0] && lights[0].error) {
      console.error(
        ` >>>>>>>>>>>>>>>>>>>>>>>>>>>>> no lights found. Error: ${JSON.stringify(
          lights,
          null,
          "    "
        )} `
...
export const getLightById = async (id: string): Promise<Light | null> => {
  ...
  if (base) {
    ...
    if (lightResult && lightResult[0] && lightResult[0].error) {
      console.error(
        ` light with ID ${id} error: ${JSON.stringify(lightResult[0].error)} `
      );
      return null;
    } else {
...
export const getGroupById = async (id: string): Promise<Room | null> => {
  ...
  if (base) {
    ...
    if (groupResult && groupResult[0] && groupResult[0].error) {
      console.error(
        ` group with ID ${id} error: ${JSON.stringify(groupResult[0].error)} `
      );
      return null;
    } else {
...
export const turnLightOff = async (
  ...
  if (base) {
    ...
    console.log(
      `switchResult[0].success: ${JSON.stringify(switchResult[0].success)}`
    );
...
export const turnGroupOff = async (
  id: string
): Promise<SuccessObject | void> => {
  if (!id) {
...
export const turnGroupOn = async (
  id: string
): Promise<SuccessObject | void> => {
  if (!id) {
...
export const setLightBrightness = async ({
  id,
  percentage,
}: SetLightBrightnessProps): Promise<SuccessObject | number | void> => {
  if (!id) {
...
export const setGroupBrightness = async ({
  id,
  percentage,
}: SetLightBrightnessProps): Promise<SuccessObject | void> => {
  if (!id) {
...
const getGroupsAsArray = (obj: any): Array<Room> => {
  const groupsArray: Array<Room> = Object.keys(obj).map((id: string) =>
    makeGroup(obj[id], id)
  );
  // if (!allGroups) {
...
const makeGroup = (group: RawGroup, id: string): Room => {
  ...
  const colorful =
    group.action && group.action.colormode && group.action.colormode === "xy";
